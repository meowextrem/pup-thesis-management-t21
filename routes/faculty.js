...
router.get('/schedules', function(req, res, next) {
  var limit = 8
  console.log(req.session.scheduleTitleQuery)
  faculty.getYears(function(years) {
    faculty.getMainThesisList(req.session.scheduleTitleQuery || "", req.session.scheduleYear || years[0].year , limit, ((req.query.p - 1)*4 || 0), function(thesisList) {
      res.render('./faculty/schedules', {
        layout: 'faculty',
        ...
        suffix: req.user.suffix,
        years: years,
        yearSelected: req.session.scheduleYear || years[0].year,
        titleQuery: req.session.scheduleTitleQuery || "",
        thesisList: thesisList,
        pagination: {
          page: req.query.p || 1,
          limit: 4,
...
router.post('/schedules', function(req, res, next) {
  ...
  if (req.body.search) {
    ...
    req.session.scheduleTitleQuery = req.body.titleQuery
    req.session.scheduleYear = req.body.year
    res.redirect('/faculty/schedules')
  } else if (req.body.clearSearch) {
    console.log('clear')
    delete req.session.scheduleTitleQuery
    delete req.session.scheduleYear
    res.redirect('/faculty/schedules')
  } else if (req.body.updateSchedule) {
    console.log('updating schedule')
    faculty.updateSchedule(req.body, function(data) {
      res.redirect('/faculty/schedules')
    })
  }
})
