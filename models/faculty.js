...
var actions = {
  ...
  getYears: (callback) => {
    const query = {
      text: `SELECT DISTINCT(EXTRACT(year FROM date_created)) as year FROM thesis ORDER BY year DESC `
    }
    db.query(query).then(data => {
      return callback(data.rows)
    }).catch(e => {
  ...
  getMainThesisList: (title, year, limit, offset, callback) => {
    const query = {
      text: `SELECT thesis.id as thesis_id, title, scheduled_date, start_time, end_time, schedules.id as schedule_id FROM thesis 
        LEFT JOIN schedules ON schedules.thesis_id = thesis.id
        WHERE title ILIKE '%${title}%' AND is_main = true  AND extract(year FROM date_created) = ${year}
        ORDER BY title ASC
        LIMIT ${limit} OFFSET ${offset}`
    }
    db.query(query).then(data => {
  ...
  updateSchedule: (body, callback) => {
    ...
    if (!body.scheduleId) {
      query = {
        text: `INSERT INTO schedules (thesis_id, scheduled_date, start_time, end_time) VALUES ($1,$2,$3,$4)`,
        values: [body.thesisId, body.date, body.startTime, body.endTime]
      }
    } else {
      query = {
        text: `UPDATE schedules SET scheduled_date = $1, start_time = $2, end_time = $3 WHERE id = $4 AND thesis_id = $5`,
        values: [body.date, body.startTime, body.endTime, body.scheduleId, body.thesisId]
      }
    }
